...
from src.config.config_v2 import team_feature_configs_v2, player_feature_configs_v2
...
class NFLModelV2:
    ...
    def build_player_feature_matrices(self, exclude_current: bool = True, include_defense: bool = True, include_differentials: bool = True):
        ...
        Must be called after build_feature_matrices().
        """
        if not hasattr(self, "_hist_df"):
        ...
        # Add defensive opponent team id (matching team features logic)
        ...
        roll_sizes = getattr(player_feature_configs_v2, 'rolling_windows', [2, 5, 10])
        
        # Helper functions (matching team features exactly)
        def prior_expanding(series: pd.Series) -> pd.Series:
            s = series.shift(1 if exclude_current else 0)
            return s.expanding(min_periods=1).mean()
        ...
        for player_rank in player_feature_configs_v2.position_rankings:
            ...
            rank_complete = game_grid.merge(
                rank_data[['season', 'week', 'gamesummaryid', 'teamid'] + stats], 
            ...
            for stat in stats:
                if stat in rank_complete.columns:
            ...
            team_grp = rank_complete.groupby('teamid', group_keys=False)
            def_grp = rank_complete.groupby('def_teamid', group_keys=False) if include_defense else None
            
            # Create features for each stat
            for stat in stats:
                if stat not in rank_complete.columns:
                    continue
