...
def assemble_matchups(artifact, team_hist: pd.DataFrame, season: int, week: int, schedule_wk: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, g in schedule_wk.iterrows():
        ...
        for base in home_feats.index:
            ...
            opp_col = f"opp_{base}"
            if diff_col in artifact.feature_names and opp_col in opp_feats.index:
    ...
    X = pd.DataFrame(columns=artifact.feature_names)
    X = pd.concat([X, frame], axis=0)
    try:
        X = X[artifact.feature_names].fillna(0)
...
def predict_target(target: str,
    ...
    # x_idx=np.where(X_test["week"]==week & X_test["season"]==season)[0]
    # x=X_test.iloc[x_idx]
    # y=y_test.iloc[x_idx]
    # dmatrix=xgb.DMatrix(x, labels=y)

    # preds = artifact.model.predict(dmatrix)
